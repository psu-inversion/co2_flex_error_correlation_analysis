...
TOWERS_LONG_DATA = [
    name for name in corr_data
    if corr_data[name].count() >= HOURS_PER_YEAR * N_YEARS_DATA and
    pair_counts.loc[
        slice(None, "{0:d} days".format(int(DAYS_PER_YEAR * N_YEARS_DATA + 1))),
        name
    ].min() >= HOURS_PER_YEAR * 0.5
]
...
uniform_corr_data.plot(
    subplots=True, sharey=True, figsize=(12.5, 6.5),
    title="AmeriFlux - CASA autocorrelations",
    xticks=pd.timedelta_range(start=0, freq="365D", periods=N_YEARS_DATA + 1).to_numpy().astype(float),
)
plt.subplots_adjust(bottom=0.07, top=0.95, left=0.05, right=0.95)
fig = plt.gcf()
for ax in fig.axes:
    ax.set_xticks(
        pd.timedelta_range(start=0, freq="365D", periods=N_YEARS_DATA + 1).to_numpy().astype(float),
        minor=False
    )
    ax.set_xticklabels(
...
uniform_spectrum = pd.DataFrame(
    ...
    index=np.fft.rfftfreq(uniform_corr_data.shape[0], 1./HOURS_PER_DAY)
)
uniform_spectrum.index.name = "Freq. (1/day)"
abs(uniform_spectrum).plot(
    subplots=True, sharey=True, logy=True, figsize=(12.5, 6.5), xlim=(0, 6),
...
uniform_spectrum_year = pd.DataFrame(
    np.fft.rfft(uniform_corr_data, axis=0),
    columns=uniform_corr_data.columns,
    index=np.fft.rfftfreq(uniform_corr_data.shape[0], 1./HOURS_PER_YEAR)
)
uniform_spectrum_year.index.name = "Freq. (1/year)"
abs(uniform_spectrum_year).plot(
    subplots=True, sharey=True, logy=True, figsize=(12.5, 6.5),
    xlim=(0, 8),
    title="AmeriFlux - CASA CO$_2$ Flux difference spectra (log scale)",
)
plt.subplots_adjust(bottom=0.07, top=0.95, left=0.05, right=0.95)
...
abs(uniform_spectrum_year).plot(
    subplots=True, sharey=True, logy=False, figsize=(12.5, 6.5),
    xlim=(0, 8), ylim=(0, 1000),
...
pca_results.scores.plot(
    subplots=True, sharey=True, ylim=(-.03, .03),
...
plt.savefig("ameriflux-minus-casa-long-correlation-spectral-pca-principal-components.png")
plt.pause(1)
