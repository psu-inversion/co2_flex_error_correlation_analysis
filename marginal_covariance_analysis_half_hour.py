...
sites_in_both = sorted(
    set(casa_ds.coords["Site_Id"].values) & set(amf_ds.coords["site"].values)
)
times_in_both = sorted(
    set(casa_ds.coords["time"].values) & set(amf_ds.coords["TIMESTAMP_START"].values)
)
amf_data = (
    amf_ds["ameriflux_carbon_dioxide_flux_estimate"]
    .sel(site=sites_in_both, TIMESTAMP_START=times_in_both)
    .stack(data_point=("site", "TIMESTAMP_START"))
    .dropna("data_point")
)
...
difference_df_rect.to_csv(
    "ameriflux-minus-casa-half-hour-towers-difference-data-rect.csv"
)
...
encoding.update(
    {name: {"_FillValue": None} for name in difference_rect_xarray.data_vars}
)

difference_rect_xarray.to_netcdf(
    "ameriflux_minus_casa_half_hour_tower_data.nc4",
    encoding=encoding,
...
site_coords = amf_ds.coords["site"].sel(site=difference_df_rect.columns)
...
length_with_nugget_opt = scipy.optimize.minimize(
    fun=lambda params, corr, dist: nansum(
        square(corr - (params[0] * exp(-dist / params[1]) + (1 - params[0])))
    ),
...
for column in acf_data.iloc[:, :]:
    ...
    for ax in axes[3:]:
        ...
        tmp = ax.set_xticks(xticklocs.to_numpy().astype("i8").astype(float))
    ...
    fig, axes = plt.subplots(
        len(CORR_FUNS) + 1, 1, figsize=(6.5, 5), sharex=True, sharey=True
    )
    ...
    tmp = fig.subplots_adjust(top=0.9, hspace=1.1)
    ...
    tmp = fig.savefig(
        "{site:s}-ameriflux-minus-casa-corr-fits-long.pdf".format(site=column)
    )
    ...
    tmp = fig.savefig(
        "{site:s}-ameriflux-minus-casa-corr-fits-short.pdf".format(site=column)
    )
    plt.close(fig)
...
for column in sorted(set(COEF_DATA.index.get_level_values(0))):
    ...
    for ax, fit_name in zip(
        ...
        if len(corr_params) == 0:
            continue
        ax.plot(acf_col.index, eval(fit_name)(acf_times, **corr_params))
...
with open("ameriflux-minus-casa-best-fits-tree.txt", "w") as out_file:
    print(
        sklearn.tree.export.export_text(
            classifier, list(sparse_explanatory_data.columns)
        ),
        file=out_file,
    )
