...
if __name__ == "__main__":
    ...
    CASA_DATA.coords["month_hour"] = CASA_DATA.coords["month"] * 1000 + CASA_DATA.coords["hour"]
    ...
    CASA_DAILY_CYCLE_BY_MONTH = (
        CASA_DAILY_CYCLE_BY_MONTH
        .unstack("month_hour")
        .set_index(ameriflux_tower_location="Site_Id")
    )

    COMBINED_TOWER_LIST = sorted(set(CASA_DATA.coords["Site_Id"].values) & set(TOWER_NAMES.values))
    ...
    for tower in COMBINED_TOWER_LIST:
        ...
        for month in range(1, MONTHS_PER_YEAR + 1):
            ...
            casa_line = ax.plot(casa_month.coords["hour"].values,
                                casa_month.values,
                                color="r", label="CASA")
            ameriflux_month=ameriflux_daily_cycles.sel(month=month)
            amf_line = ax.plot(ameriflux_month.coords["hour"].values, 
                               ameriflux_month.values,
                               color="b", label="AmeriFlux")
        ...
        fig.suptitle("CASA and AmeriFlux NEE at {site:s}"
                     .format(site=casa_daily_cycles.coords["Name"].values))
        fig.subplots_adjust(bottom=.16, hspace=.3)
        fig.legend([casa_line[0], amf_line[0]], ["CASA", "AmeriFlux"],
